...
fn mcp_advisor_snapshot_omits_server_health_for_per_call_context_override() {
    ...
    let temp_dir = std::env::temp_dir().join(format!(
        "pozsar_mcp_advisor_snapshot_override_{}",
        std::process::id()
    ));
    std::fs::create_dir_all(&temp_dir).unwrap();
    let server_context_path = temp_dir.join("server_market_context.json");
    let server_health_path = temp_dir.join("server_market_context.health.json");
    let override_context_path = temp_dir.join("override_market_context.json");
    std::fs::write(
        &server_context_path,
        r#"{
  "as_of": "2026-06-30",
  ...
  "missing_assets": [],
  "stale_assets": [],
  "warnings": ["server health should not be reused"],
  "blocking_issues": []
}"#,
    )
    .unwrap();
    std::fs::write(
        &override_context_path,
        r#"{
  "as_of": "2026-07-01",
  "assets": [
    {
  ...
  "cross_asset": {
    ...
    .unwrap();
    let service = PozsarCorpusMcp::new(chunks)
        .with_market_context_path(server_context_path.display())
        .with_market_context_health_path(server_health_path.display());

    let response = service.get_pozsar_advisor_snapshot(Parameters(AdvisorSnapshotParams {
        question: "What does collateral scarcity imply for BTC?".to_string(),
        assets: vec!["BTC".to_string()],
        themes: Some(vec![
            "collateral".to_string(),
            "dollar_liquidity".to_string(),
        ...
        market_context_path: Some(override_context_path.display().to_string()),
        market_context_health_path: None,
        limit: Some(4),
    }));
    let snapshot: Value = serde_json::from_str(&response).unwrap();

    assert_eq!(snapshot["market_context"]["as_of"], "2026-07-01");
    assert!(snapshot.get("market_context_health").is_none());
}

#[test]
fn mcp_advisor_snapshot_rejects_mismatched_market_context_health() {
    let chunks = vec![chunk(
        "doc",
        "Liquidity.pdf",
        1,
    ...
    let temp_dir = std::env::temp_dir().join(format!(
        "pozsar_mcp_advisor_snapshot_mismatch_{}",
        std::process::id()
    ));
    std::fs::create_dir_all(&temp_dir).unwrap();
    let market_context_path = temp_dir.join("market_context.json");
  ...
  "blocking_issues": []
    ...
    assert!(response["error"].as_str().unwrap().contains(
        "market context health as_of 2026-06-29 does not match market context as_of 2026-06-30"
    ));
    assert!(response.get("market_context_health").is_none());
}
